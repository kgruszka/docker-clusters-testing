...
test_dir="../tests/start_containers_test"
...
if [ -z "$DEBUG" ] ; then
  ...
  cd terraform
  TF_VAR_local_test_dir_path=$test_dir TF_VAR_local_test_dir_path=$test_dir TF_VAR_storage_path=$storage_path TF_VAR_master_node_count=$master_nodes TF_VAR_slave_node_count=$slave_nodes TF_VAR_containers_count=$containers_count terraform destroy -force -target=module.swarm
  cd ../.
fi
